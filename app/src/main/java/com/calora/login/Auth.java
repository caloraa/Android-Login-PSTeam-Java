...
public class Auth extends AsyncTask<String, Void, String> {
    ...
    private  String a = "a";
    //private String URLSERVER = "https://vipmodgame.tk/login.php";
   ...
   // public native void Check();
    ...
    protected void onPostExecute(String s) {
        ...
        if(s == null || s.isEmpty()){
            Toast.makeText(activity,(Html.fromHtml("<font face='monospace'> <font color='#00ff00'>Server Error!</font></font>")), Toast.LENGTH_LONG).show();
            return;
        }
        ...
        try {
            ...
            if (!verify(decData, ack.get("Sign").toString(), puk)) {
                Toast.makeText(activity, (Html.fromHtml("<font face='monospace'> <font color='#00ff00'>Dados Incorretos!</font></font>")), Toast.LENGTH_LONG).show();
                return;
            }
            ...
            if(data.get("Status").toString().equals("Success")) {
               ...
               .setMessage("\nUsuario:" + data.get("Username").toString() + "\nValidade:" + data.get("SubscriptionLeft").toString().replace(" 00:00:00", "") 
                        ...
                        alertDialog.show();
                 Toast.makeText(activity,Html.fromHtml("<b><font color='green'>Conectado! " + data.get("Dias").toString() + "</font>"), Toast.LENGTH_LONG).show();
                //getActivity().startActivity(new Intent(activity, Class.forName(getActivity().sGameActivity)));
                //System.loadLibrary("MyLibName");
               // Check();
                if (data.get("Status").toString().equals("Success")) {
                ...
                Toast.makeText(activity,Html.fromHtml("<font color='#00ff00'>" + data.get("MessageString").toString() + "</font>"), Toast.LENGTH_LONG).show();
            }
        } catch (Exception e) {
            e.printStackTrace();
